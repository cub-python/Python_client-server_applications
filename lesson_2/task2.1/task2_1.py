...
def get_data():
    ...
    for i in files:
        with open(i, encoding='cp1251') as file_in:
            input = file_in.read()
        os_prod_list.append(','.join(re.findall(file_template[0], input)))
        os_name_list.append(','.join(re.findall(file_template[1], input)))
        os_code_list.append(','.join(re.findall(file_template[2], input)))
        os_type_list.append(','.join(re.findall(file_template[3], input)))
